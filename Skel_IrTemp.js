...
var fs = require("fs");
fs.writeFile("SensorData.csv",'"CurrentTime","lux","humidity","temperatureFromHumidity","BarometricPressure","objectTemperature","Ir Temperature"\n');
    ...
    SensorTag.discoverAll(function (sensorTag) {
        ...
        sensorTag.connectAndSetUp(function (error) {
            ...
            Async.series([
                function (callback)
                {
                    console.log("Starting sensing all the data for %s...", sensorTag.id);
...
fs.appendFile("SensorData.csv",readings.currentTime+","+readings.lux+","+readings.humidity+","+readings.temperatureFromHumidity+","+readings.pressure+","+readings.objectTemperature+","+readings.temperatureFromIr+"\n");
                        console.log("Data recorded on %s...", readings.currentTime);
                                                      //Azure
                    });
                }, 397);                        //Sensing Time Interval in milliseconds
   setInterval(function(){
var child = exec('sudo Rscript training_vectors.R');   //Calling the R Script
...
},20000);                              //Processing Time Interval in milliseconds 2973
            });
        });
    });
});
