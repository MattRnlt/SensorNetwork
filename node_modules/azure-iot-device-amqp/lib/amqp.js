...
var Amqp = /** @class */ (function (_super) {
    ...
    function Amqp(authenticationProvider, baseClient) {
        ...
        _this._amqp = baseClient || new azure_iot_amqp_base_1.Amqp(false, 'azure-iot-device/' + packageJson.version);
        ...
        _this._fsm = new machina.Fsm({
            ...
            states: {
                disconnected: {
                    ...
                    sendEvent: function (message, sendCallback) {
                        ...
                        _this._fsm.handle('connect', function (err, result) {
                            if (err) {
                                ...
                                _this._fsm.handle('sendEvent', message, sendCallback);
                            }
                        });
                    },
                    ...
                    _onEnter: function (connectCallback) {
                        ...
                        _this._authenticationProvider.getDeviceCredentials(function (err, credentials) {
                            if (err) {
                                ...
                                _this._fsm.transition('disconnected', connectCallback, azure_iot_amqp_base_1.translateError('AMQP Transport: Could not get credentials', err));
                            }
                            else {
                                _this._c2dEndpoint = azure_iot_common_1.endpoint.messagePath(encodeURIComponent(credentials.deviceId));
                                _this._d2cEndpoint = azure_iot_common_1.endpoint.eventPath(credentials.deviceId);
                                var uri = _this._getConnectionUri(credentials.host);
                                _this._amqp.connect(uri, credentials.x509, function (err, connectResult) {
                                    if (err) {
                                        _this._fsm.transition('disconnected', connectCallback, azure_iot_amqp_base_1.translateError('AMQP Transport: Could not connect', err));
                                    }
                                    else {
                                        _this._fsm.transition('authenticating', connectCallback, connectResult);
                                    }
                                });
                            }
                        });
                    },
                    disconnect: function (disconnectCallback, err) { return _this._fsm.transition('disconnecting', disconnectCallback, err); },
                    updateSharedAccessSignature: function (token, callback) {
                        callback(null, new azure_iot_common_1.results.SharedAccessSignatureUpdated(false));
                    },
                ...
                authenticating: {
                    _onEnter: function (connectCallback, connectResult) {
                        if (_this._authenticationProvider.type === azure_iot_common_1.AuthenticationType.X509) {
                            ...
                            _this._fsm.transition('authenticated', connectCallback, connectResult);
                        }
                        else {
                            _this._amqp.initializeCBS(function (err) {
                                if (err) {
                                    ...
                                    _this._fsm.transition('disconnecting', connectCallback, getTranslatedError(err, 'AMQP Transport: Could not initialize CBS'));
                                }
                                else {
                                    _this._authenticationProvider.getDeviceCredentials(function (err, credentials) {
                                        if (err) {
                                            _this._fsm.transition('disconnecting', connectCallback, getTranslatedError(err, 'AMQP Transport: Could not get credentials from AuthenticationProvider'));
                                        }
                                        else {
                                            ...
                                            _this._amqp.putToken(azure_iot_common_1.SharedAccessSignature.parse(credentials.sharedAccessSignature, ['sr', 'sig', 'se']).sr, credentials.sharedAccessSignature, function (err) {
                                                if (err) {
                                                    ...
                                                    _this._fsm.transition('disconnecting', connectCallback, getTranslatedError(err, 'AMQP Transport: Could not authorize with puttoken'));
                                                }
                                                else {
                                                    _this._fsm.transition('authenticated', connectCallback, connectResult);
                                                }
                    ...
                    disconnect: function (disconnectCallback) { return _this._fsm.transition('disconnecting', disconnectCallback); },
                    '*': function () { return _this._fsm.deferUntilTransition(); }
                },
                authenticated: {
                    _onEnter: function (connectCallback, connectResult) {
                    ...
                    sendEvent: function (message, sendCallback) {
                        ...
                        amqpMessage.properties.to = _this._d2cEndpoint;
    ...
    Amqp.prototype._getConnectionUri = function (host) {
        return 'amqps://' + host;
    };
